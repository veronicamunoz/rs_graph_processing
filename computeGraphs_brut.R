...
compute_Graph_brut <- function(name.control,rs_path,atlas_file,graph_path,file_coord, compute_cor = TRUE, graphs = TRUE, regions_selected=NULL,percentage_selected=4, num.levels,num.nb.edges,length_time_series=NULL)
{
  list_valide_patient <- vector()
  ...
  ######### DOSSIER DE SORTIE
  dir.create(file.path(graph_path, "Graph_Brut"), showWarnings = FALSE)
  dir.create(file.path(paste(graph_path,'Graph_Brut',sep='/'), templBaseName), showWarnings = FALSE)
  name.dir<-paste(graph_path,'Graph_Brut', templBaseName,sep='/')
  ...
  cor.data <- cor(data.roi)
  browser()
  for(j in 1:n.regions){
    if(sum(data.roi[,j])==0){
      warning(paste('problem region',j,'equal to 0'))
      if(j ==1){
        cor.data[,j]<-c(1,rep(0,length((j+1):n.regions)))
        cor.data[j,]<-c(1,rep(0,length((j+1):n.regions)))
      }
      else if(j==n.regions){
        cor.data[,j]<-c(rep(0,length(1:(j-1))),1)
        cor.data[j,]<-c(rep(0,length(1:(j-1))),1)
      }
      else{
        cor.data[,j]<-c(rep(0,length(1:(j-1))),1,rep(0,length((j+1):n.regions)))
        cor.data[j,]<-c(rep(0,length(1:(j-1))),1,rep(0,length((j+1):n.regions)))
      }				
    }
  }
  ...
  write.table(cor.data,paste(name.dir,'/','/','wave.cor.mat_n.regions_',n.regions,'_start_time_series_',length_time_series[1],'proc_length',proc.length,'.grey.matter.txt',sep=''),col.names=F,row.names=F,quote=F)
  CairoPNG(file=paste(name.dir,'/','/','Corr.mat_n.regions_',n.regions,'_start_time_series_',length_time_series[1],'proc_length',proc.length,'.grey.matter.png',sep=''),width = 1000, height = 800,family='times')
  col1 <- colorRampPalette(c("#00007F", "blue", "#007FFF", "cyan", "yellow",  "#FF7F00", "red", "#7F0000"))
  corrplot(cor.data, method="color", col = col1(100))
  dev.off()
  ...
  if(graphs){
    ##browser()
    cor.mat<-read.table(paste(name.dir,'/','wave.cor.mat_n.regions_',n.regions,'_start_time_series_',length_time_series[1],'proc_length',proc.length,'.grey.matter.txt',sep=''))
    cat(paste(name.dir,'/','wave.cor.mat_n.regions_',n.regions,'_start_time_series_',length_time_series[1],'proc_length',proc.length,'.grey.matter.txt',sep=''),'\n') 
    ...
    for(i in cost){
      if (i<=corr_zero){ #MTC: Remove this line to allow graphs without significance constrain
        ...
        if(corr_zero>cost[percentage_selected]){
          list_valide_patient<-c(list_valide_patient,name.control)
